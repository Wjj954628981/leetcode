...
//            System.out.println(i + "===" + val + "===" + ret + "===" + left + "===" + map.toString());
    ...
    public void dfs(TreeNode node) {
        ...
        if(node.left == null && node.right == null){
            maxDiff = Math.max(maxDiff, Collections.max(onePath) - Collections.min(onePath));
        }
        dfs(node.left);
        dfs(node.right);
    ...
    private int maxdiff(TreeNode root, int min, int max){
        ...
        return Math.max(Math.max(Math.abs(root.val-min), Math.abs(root.val-max)), Math.max(left, right));
    }
    ...
    private boolean bstCheck(TreeNode node, Integer min, Integer max){
    ...
    public ListNode oddEvenList(ListNode head) {
        ...
        ListNode o_end = head, e_head = head.next, e_end = e_head;
        while(o_end.next != null && e_end.next != null){
            o_end.next = e_end.next;
            o_end = o_end.next;
            e_end.next = o_end.next;
            e_end = e_end.next;
        }
        o_end.next = e_head;
        return head;
